# -*- coding: utf-8 -*-
"""
Created on Fri Apr  2 14:26:14 2021
...
train = pd.read_csv(r'D:\Task\Analytics_Vidhya\Customer_Segmentation\Train_aBjfeNk.csv')
train = train.fillna('')
#df = train.copy(deep=True)
df =train
test = pd.read_csv(r'D:\Task\Analytics_Vidhya\Customer_Segmentation\Test_LqhgPWU.csv')
test = test.fillna('')
...
test_1 = testx[['ID' , 'Segmentation']

#test_1['Segmentaion'].fillna(new_preds2)

df_nan = test_1.dropna()

for i,j in  zip(l, new_preds2):
    if [i] == np.nan:
        print([i])
    
df_POL=pd.concat([test_1,df_nan]).drop_duplicates(keep=False) 
      

df_submit = pd.DataFrame({'ID': df_POL['ID'].values, 'Segmentation':new_preds2})

df_fsubmit = pd.concat([df_nan,df_submit])

df_fsubmit.to_csv('submit4.csv', index = False)
